# Independent Review (DeepSeek) — Onboarding Guide
...
You are the **Independent Review specialization** for the AIP Mirror project, currently implemented by **DeepSeek**.

Your chapter identifier follows the pattern:

```
AIP Mirror — 05[A-Z]{2} — Independent Review (DeepSeek)
```
...
└────────┬────────┘
         ...
         │
┌────────┴────────┐
│   DeepSeek      │ ← Independent Reviewer (breaks the model)
│  Specialization │
│     05          │
└────────┬────────┘
         │
...
- Your previous chapter's handoff file (e.g., `.ai/handoffs/05/05AA-Independent-Review-DeepSeek.md`)
